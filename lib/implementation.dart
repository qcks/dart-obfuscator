...
Structure determineStructure(Directory libDir, String sourceDirPath) {
  ...
  rawFiles.removeWhere(
      (element) => element.path == "${libDir.path}/$outputFileName");
...
List<File> findFilesToObfuscate(
        Directory libDir, List<FileSystemEntity> rawFiles) =>
    libDir
...
String scrapCodeToObfuscate(
    List<File> filesToObfuscate, Directory libDir, String outputFileName) {
...
bool isImportOfFileToBeDeleted(
    String absoluteImport, List<File> filesToObfuscate) {
...
void updateImportsInNonObfuscatedFiles(
    Structure structure, String outputFileName) {
  structure.rawFiles.forEach((file) {
    ...
    if (logLevel == LogLevel.VERBOSE)
      print(
          "Update ${lines.length - updatedLines.length} imports for file: ${outputFilePath}");
...
List<File> findExportedFiles(List<FileSystemEntity> rootFiles) {
  ...
  rootFiles.forEach((element) {
    ...
    final exportFiles = checkExports(File(element.path));
    if (logLevel == LogLevel.VERBOSE) {
...
String renameClasses(String codeToObfuscate, List<String> mappingSymbols,
    Map<String, String> resultingMapping) {
  ...
  classNames.forEach((theClass) {
    final theMapping =
        mappingSymbols.removeAt(Random().nextInt(mappingSymbols.length));
...
void updateRawFilesWithObfuscatedClasses(
    List<File> rawFiles, Map<String, String> resultingMapping) {
...
void deleteEmptyDirectories(Directory libDir) {
  libDir
      .listSync(recursive: true)
      .whereType<Directory>()
      .where((element) => element.listSync(recursive: true).isEmpty)
      .forEach((emptyDir) {
    emptyDir.deleteSync();
  });
}
